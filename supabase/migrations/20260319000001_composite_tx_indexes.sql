...
--
-- Uses CONCURRENTLY to avoid blocking reads/writes on a live table.
-- Requires `-- no-transaction` above because CONCURRENTLY cannot
-- run inside a transaction.

DROP INDEX CONCURRENTLY IF EXISTS idx_blockchain_tx_from_address;
DROP INDEX CONCURRENTLY IF EXISTS idx_blockchain_tx_to_address;
DROP INDEX CONCURRENTLY IF EXISTS idx_blockchain_tx_contract_hash;

CREATE INDEX CONCURRENTLY idx_blockchain_tx_from_address
    ON blockchain_transactions (from_address, block_number DESC NULLS LAST);

CREATE INDEX CONCURRENTLY idx_blockchain_tx_to_address
    ON blockchain_transactions (to_address, block_number DESC NULLS LAST);

CREATE INDEX CONCURRENTLY idx_blockchain_tx_contract_hash
