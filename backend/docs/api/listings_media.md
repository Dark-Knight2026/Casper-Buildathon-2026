...
- `authorityTier` is one of `T0` (self-attested), `T1` (documents on file), `T2` (source-verified; deferred). The wire form keeps the uppercase `T0`/`T1`/`T2`, not snake_case.
...
- **Behavior:**
  ...
  - **Moderation state.** A newly uploaded item is auto-approved by default, so it is publicly visible immediately; an Agent may still `reject` it post-hoc via the moderation endpoint. Position is the next free slot (appended after existing media).
