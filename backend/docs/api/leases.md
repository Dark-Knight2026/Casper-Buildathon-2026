...
```json
{
  ...
  "invoiceValidityDuration": 604800
}
```

- `onchainLeaseId` is the U256 agreement id (decimal string) from `LeaseAgreementCreated`; it must be non-empty and all digits. `0` is a **valid** id - the contract assigns ids from `get_lease_agreements_count()` before incrementing, so the first lease on chain has id `0`.
...
- `invoiceValidityDuration` is the seconds the landlord passed as `invoice_validity_duration` to `create_lease_agreement`; the backend uses it to compute the off-chain invoice deadlines it seeds at commit (see behavior below).
...
- **Input:** `CommitLeaseRequest` (`{ onchainLeaseId, nftTokenId, commitTxHash, invoiceValidityDuration }`).
- **Response (200):** the `Lease`, moved to `active` with its on-chain bindings persisted.
- **Behavior:** records the on-chain result of `create_lease_agreement` and activates the lease. Landlord-only, owner-only, and blocked until both consent signatures are present. Validates the ids (decimal `onchainLeaseId`, including `0`; decimal `nftTokenId`), then reconciles against the chain via `LeaseChainReader` - the agreement must exist, be unfinished, have its security deposit paid, and have all invoices paid - before persisting `onchainLeaseId`/`nftTokenId`/`commitTxHash` and moving `pending_signatures -> active`. Activation and invoice seeding share **one transaction**: within the same `BEGIN`/`COMMIT` the handler seeds one `security_deposit` plus N `rent` invoice mirrors (one per 30-day month) in `scheduled` status, with deadlines derived from `NOW()` offset by `invoiceValidityDuration`, so a lease can never be `active` without its invoice rows (see [Invoices API](./invoices.md)). The seeding is idempotent. **Idempotent overall:** if the indexer's `LeaseAgreementCreated` handler already activated the lease (even between this handler's read and write), the active lease is returned unchanged.
- **Errors:** 400 (empty/non-decimal `onchainLeaseId`, non-decimal `nftTokenId`, or id absent on-chain), 401, 403 (not the lease landlord), 404 (lease not found), 409 (not awaiting commit, missing signatures, not fully funded, or already finished on-chain), 500 (on-chain transport failure)
- **Auth:** Access cookie required; landlord role, owner only.
