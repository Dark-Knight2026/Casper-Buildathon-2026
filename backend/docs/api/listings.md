...
Returned by the detail, create, update, submit, and state-change endpoints, and as each item of the search / landlord lists. The nested `property` is present in detail and list responses; `media` is approved-only in public reads. Wire names are `camelCase`; `onChain` is always `null` for now.
