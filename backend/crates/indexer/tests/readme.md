...
| ces_parser.rs               | Unit tests for `parse_ces_event`: CES binary blob decoding per contract schema               |
...
| property_registry_events.rs | Integration tests for `PropertyRegistry` `PropertyCreated`: metadata_uri match, tokenization |
...
| user_registry_events.rs     | Integration tests for `UserRegistry` `UserCreated`: wallet match, `onchain_user_id` write    |
