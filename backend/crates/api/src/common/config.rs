...
pub struct AppState {
    ...
    /// Boxed-trait so the bootstrap can swap `FakeKycProvider` for
    ...
    /// the bootstrap can swap `StubFairHousingScreen` for the
    ...
    /// bootstrap can swap `SimpleLeaseDocumentRenderer` for a real
    ...
    /// so the bootstrap can swap `FakeBackgroundCheckProvider` for
