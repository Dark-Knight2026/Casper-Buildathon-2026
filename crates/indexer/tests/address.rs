...
use indexer::address::normalize_to_account_hash;

#[test]
fn empty_string_passes_through() {
    assert_eq!(normalize_to_account_hash("").unwrap(), "");
}
...
fn ed25519_public_key_66_hex() {
    ...
    let result = normalize_to_account_hash(pk_hex).unwrap();
...
fn idempotent_for_64_hex() {
    ...
    let first = normalize_to_account_hash(input).unwrap();
    let second = normalize_to_account_hash(&first).unwrap();
    assert_eq!(first, second, "normalization must be idempotent");
}

#[test]
fn rejects_garbage_input() {
    assert!(normalize_to_account_hash("not-a-valid-address").is_err());
}
...
fn rejects_wrong_length_hex() {
    ...
    assert!(
        normalize_to_account_hash("aabbccddeeff00112233445566778899aabbccddeeff001122334455")
            .is_err()
    );
}
