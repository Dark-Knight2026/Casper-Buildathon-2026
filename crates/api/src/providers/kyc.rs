...
//! reports verified, used for the hackathon. Production deployments swap in a
...
pub trait KycProvider: Send + Sync {
    ...
    /// from `user_id` and runs its checks; the hackathon fake ignores the
...
/// Hackathon implementation that always reports verified.
///
