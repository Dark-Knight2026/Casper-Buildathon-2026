pub const ONE_HUNDRED_PERCENT_BPS: u16 = 10_000; // 100.00%

pub const STAKING_REWARDS_BPS: u16 = 4_000; // 40.00%

pub const INCENTIVES_REWARDS_BPS: u16 = 6_000; // 60.00%

pub const ONE_MONTH_IN_SECONDS: u64 = 30 * 24 * 60 * 60;

pub const STYKS_ORACLE_CSPR_USDT_PRICE_FEED_ID: &'static str = "CSPRUSD";

pub const SIX_MONTHS_MS: u64 = 6 * 30 * 24 * 60 * 60 * 1_000;

pub const TWELVE_MONTHS_MS: u64 = 12 * 30 * 24 * 60 * 60 * 1_000;

pub const PRIVATE_SALE_CLIFF_DURATION: u64 = SIX_MONTHS_MS;

pub const PRIVATE_SALE_VESTING_DURATION: u64 = TWELVE_MONTHS_MS;
