/**
 * Optimized Enhanced Lease Generation Wizard
...
const PropertyDetailsStep = lazy(() => import('./wizard-steps/PropertyDetailsStep'));
const PartiesInformationStep = lazy(() => import('./wizard-steps/PartiesInformationStep'));
const LeaseTermsStep = lazy(() => import('./wizard-steps/LeaseTermsStep'));
const FinancialTermsStep = lazy(() => import('./wizard-steps/FinancialTermsStep'));
const ClausesStep = lazy(() => import('./wizard-steps/ClausesStep'));
const ReviewStep = lazy(() => import('./wizard-steps/ReviewStep'));

// Lazy load signature workflow
const SignatureWorkflowManager = lazy(() => import('./signing/SignatureWorkflowManager'));

interface EnhancedLeaseWizardOptimizedProps {
  templateId?: string;
...
export default function EnhancedLeaseWizardOptimized({
  templateId,
  existingLeaseId,
  onComplete,
  onCancel
}: EnhancedLeaseWizardOptimizedProps) {
  ...
  const checkCompliance = useCallback(async () => {
    const endMeasure = performanceMonitor.startMeasure('wizard_check_compliance');
    ...
    try {
      ...
      endMeasure({ score: validation.complianceScore, isValid: validation.isValid });
    } catch (error) {
  ...
  const handleGenerate = useCallback(async () => {
    const endMeasure = performanceMonitor.startMeasure('wizard_generate_lease');
  ...
  }, [generatedLease, generateLease, toast]);
  ...
  }, [generatedLease, generateLease]);

  const handleSignatureComplete = useCallback((certificateId: string) => {
    toast({
  ...
  }, [toast, onComplete, generatedLease]);
