...
import { useClientLandlord } from '@/contexts/ClientLandlordContext';
import { ClientFormData, ContactDetails, PropertyPreferences, ClientSegmentation, Client } from '@/types/client';
...
export default function ClientProfileForm({ clientId, onSave, onCancel }: ClientProfileFormProps) {
  const { createClient, updateClient, getClientById, properties } = useClientLandlord();
  ...
  const [formData, setFormData] = useState<ClientFormData>({
    ...
    propertyId: existingClient?.propertyId || '',
  ...
  // Get available properties (not assigned to other landlords)
  const availableProperties = properties.filter(p => 
    !p.landlordId || p.landlordId === clientId || p.id === formData.propertyId
  );
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {clientId ? 'Edit Client Profile' : 'Create New Client Profile'}
        </CardTitle>
      </CardHeader>
      <CardContent>
                  ...
                  <SelectContent>
                    ...
                    {availableProperties.map((property) => (
                      <SelectItem key={property.id} value={property.id}>
                        {property.details.address.street}, {property.details.address.city} - ${property.details.price.toLocaleString()}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <p className="text-sm text-gray-600 mt-1">
                  Each landlord can only be assigned to one property. Only available properties are shown.
                </p>
              </div>
