...
describe('VerifyEmail', () => {
  ...
  it('renders invalid_token (not needs_login) and does NOT stash intent on a bad-token 401 (AUTH-12 / AUTH-01)', async () => {
    confirmEmailVerification.mockRejectedValue(
      apiError(401, 'invalid_or_expired_token')
    );
    renderPage('tok');
    await waitFor(() =>
  ...
  it('offers a working resend button in bad_format that calls sendVerificationEmail (VE-01)', async () => {
    ...
    const resendBtn = await screen.findByRole('button', {
      name: /send a new verification email/i,
    });
    ...
    await waitFor(() =>
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: expect.stringMatching(/verification email sent/i),
        })
      )
    );
  });
  ...
  it('shows a rate-limit toast when resend is throttled with 429 (VE-01b)', async () => {
    ...
    await waitFor(() =>
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: expect.stringMatching(/please wait/i),
          variant: 'destructive',
        })
      )
    );
  ...
  it('shows a generic error toast when resend fails unexpectedly (VE-01b)', async () => {
    ...
    await waitFor(() =>
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: expect.stringMatching(/could not send email/i),
          variant: 'destructive',
        })
      )
    );
  ...
  it('shows no_token when the link has no token', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/needs a verification token/i)
      ).toBeInTheDocument()
    );
  ...
  it('routes a 500 to generic_error with both Retry and Resend — TBLK-03', async () => {
    ...
    expect(
      screen.getByRole('button', { name: /new email/i })
    ).toBeInTheDocument();
  });
  ...
  it('retries the verification when Retry is clicked from generic_error — TBLK-03', async () => {
    ...
    await waitFor(() =>
      expect(confirmEmailVerification).toHaveBeenCalledTimes(2)
    );
  });
